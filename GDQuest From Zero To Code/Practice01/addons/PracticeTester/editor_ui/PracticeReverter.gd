...
func revert(practice_scene_path: String) -> bool:
	...
	var success := false
	success = _delete_directory_contents(scene_directory)
	if not success:
		return false
...
# Recursively deletes all files and sub-directories in `dirpath`.
# Returns true if the operation succeeded.
func _delete_directory_contents(dirpath: String) -> bool:
	...
	while file_name != "":
		...
		if directory.current_is_dir():
			...
			_delete_directory_contents(path)
