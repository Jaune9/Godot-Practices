...
# Merges completed entries in profile_data with the new practices_data entries.
func update_practices_list(practices_data: Dictionary) -> void:
	...
	for practice_name in practices_data:
		var dict: Dictionary = practices_data[practice_name]

		profile_data[practice_name] = {
			project = dict.project,
			scene = dict.scene,
			completed = practice_name in completed_practices,
			}

		var scene_path: String = dict.scene
		practice_file_to_name[scene_path] = practice_name
