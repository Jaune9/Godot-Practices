...
func setup(practices_data: Dictionary) -> void:
	if ResourceLoader.exists(FILE_PATH):
		_load(false, practices_data)
	else:
		_create(practices_data)
...
# profile.
func _load(force_reload := false, practices_data := {}) -> void:
	...
	if practices_data:
		var difference := practices_data.size() - _loaded_profile.profile_data.size()
