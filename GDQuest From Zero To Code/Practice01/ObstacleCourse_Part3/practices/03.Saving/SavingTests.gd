...
func _init() -> void:
	...
	add_requirement("HBoxContainer/SaveButton", [], [], ["pressed"])
	add_requirement("HBoxContainer/AddItemButton", [], [], ["pressed"])
	add_requirement("HBoxContainer/RemoveItemButton", [], [], ["pressed"])
...
func test_saving_function_exists() -> String:
	if not inventory.has_method("save"):
		return tr("There's no `save` function on inventory. Did you create the proper function?")
	return ""

func test_saving_function_works() -> String:
	if inventory.has_method("save"):
